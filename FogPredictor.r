...
metar <- read.csv(file = "YPPH.txt", header = TRUE, skip = 5)
# metar <- read.csv(file = "YBBN.txt", header = TRUE, skip = 5)
...
# # create vector with 1 if fog occured, 0 otherwise
# require(parallel)
...
#       , mc.preschedule = TRUE
#       )))
# save(FG_or_BR_v, file = paste("FG_or_BR_v", metar$station[1], ".RData", sep = ""))

load(file = paste("FG_or_BR_v", metar$station[1], ".RData", sep = ""))
...
save(first_fog_time_v, file = paste("first_fog_time_v", metar$station[1], ".RData", sep = ""))

load(file = paste("first_fog_time_v", metar$station[1], ".RData", sep = ""))

# merge fog_start_time, FG_or_BR_v into metar
require(data.table)
metar <- as.data.table(metar)
metar <- metar[data.table(date = unique(metar$date), first_fog_time = first_fog_time_v, FG_or_BR = FG_or_BR_v), on= "date"]
metar <- as.data.frame(metar)
metar$valid <- as.POSIXct(metar$valid, tz = "UTC", origin = "1970-01-01")
metar$first_fog_time <- as.POSIXct(metar$first_fog_time, tz = "UTC", origin = "1970-01-01")

# save metar
save(metar, file = paste("metar_", metar$station[1], ".RData", sep = ""))

# load metar
load(file = paste("metar_", metar$station[1], ".RData", sep = ""))

# create minutes until fog
metar$minutes_until_fog <- as.numeric(ceiling((metar$first_fog_time - metar$valid)/60))
head(metar)

# subset to minutes until fog > leadtime
metar_leadtime <- metar[metar$minutes_until_fog >= leadtime*60 & metar$minutes_until_fog <= (leadtime + 3)*60, ]
...
# save data_train
save(data_train, file = paste("data_train", metar$station[1], ".RData", sep = ""))

# load data_train
load(file = paste("data_train", metar$station[1], ".RData", sep = ""))
